...
namespace Widgets
{
    class ITB : Widget
    {
        protected override void DrawSelf(Microsoft.Xna.Framework.Graphics.SpriteBatch sb)
        {
            ...
            if (!lp.dead) sb.Draw(GetTexture("Widgets/sprites/wp"), new Rectangle((ci.itbi ? 22 : 4) + (int)tp.X, (ci.itbi ? 6 : 4) + (int)tp.Y, (int)Mod0.SD(50 * lp.immuneTime, 0 == MP.it ? lp.immuneTime + Mod0.tit : MP.it), 10), Gradient(ci.itbrg, ci.itbsc, ci.itbec, Mod0.SD(lp.immuneTime, 0 == MP.it ? lp.immuneTime + Mod0.tit : MP.it)));
